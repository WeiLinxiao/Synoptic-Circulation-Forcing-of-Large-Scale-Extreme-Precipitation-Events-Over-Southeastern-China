...
# -*- coding: utf-8 -*-
"""
Created on Tue May  9 15:45:14 2023
...
# -*- coding: utf-8 -*-
"""
Created on Mon May  8 22:39:48 2023
...
#%%
mf_v=xr.open_dataarray(basic_dir+'research/6.Southeastern_China_Clustering/data/ERA5_variables/ERA5_19790101_20201231_daily_moisture_flux_integraded_v_component.nc')
mf_u=xr.open_dataarray(basic_dir+'research/6.Southeastern_China_Clustering/data/ERA5_variables/ERA5_19790101_20201231_daily_moisture_flux_integraded_u_component.nc')
...
d3_events_stats = np.load(basic_dir+'/research/6.Southeastern_China_Clustering/code/4.3d_event_indentify/d3_events_stats_combineislands.npy',allow_pickle=True).tolist()
event_sum_original = d3_events_stats.get('event_sum')
#Summary_Stats = d3_events_stats.get('area_Summary_Stats')

events_stats = np.load(basic_dir+'/research/6.Southeastern_China_Clustering/code/5.largescale_exP_events_circulation_stats/events_stats_combineislands.npy',allow_pickle=True).tolist()
event_all_stats = events_stats.get('event_all_stats')
event_basic_stats = events_stats.get('event_basic_stats')
 
tcp_events = pd.read_csv('/media/dai/DATA1/research/6.Southeastern_China_Clustering/code/7.tc_track/tcp_events.csv',index_col=0)
...
for j in [0,1,2,3,4,5]:
    ...
    z500_mean = z500.sel(time =time).mean('time')
    ...
    ivtq = ax.quiver(  mf_u['longitude'].values, mf_u['latitude'].values, 
                 ...
                 headwidth=6,
                 color='grey',zorder=100,scale_units = 'inches',
                 angles = 'uv', regrid_shape = 60,scale=1400)
    ...
    cont = ax.contour(z500_mean['longitude'].values, z500_mean['latitude'].values, 
               ...
               levels=[5580,5620,5660,5700,5740,5780,5820,5860],
    ...
    ax.clabel(cont, inline=False, fontsize=6, fmt='%d',colors='black')
    ...
    ax.set_title(r"$\bf{("+title_aux[j]+")}$ Cl. "+ str(j+1)+" ("+str(np.round(count_f[sequence_f[j]],1))+"%)",
    ...
    '''
fig.savefig("/media/dai/DATA1/research/6.Southeastern_China_Clustering/code2/Figures/Figure.Sx.climatological_Z500_IVT_type_in_all_days.png",dpi=1500, bbox_inches='tight')
