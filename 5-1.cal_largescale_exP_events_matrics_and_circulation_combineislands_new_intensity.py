    ...
    for ar in [0.95]:
        ...
        for r in [3]:
            for vv in ['SLP~Z500']:
                for f in [6]:
                    ...
                    percent_event_ori = pd.DataFrame()
                    s=0
                    for i in range(len(event_sum_original)):
                        ...
                        percent_event_ori = pd.concat([percent_event_ori,   p1  ])
                        
                        if p1.sum() == p2.sum():
                            s=s+1
                        elif len(p1)-(p1==p1.value_counts().index[0]).sum()<=1:
                            s=s+1
                        elif (len(p1)>2) & ((p1==p1.value_counts().index[0]).sum()>=2):
                            s=s+1
        ...
        for i in range(len(event_sum_original)):
            ...
            for j in range(len(np.unique(a.iloc[:,0]))): 
                ...
                p_event =np.full((140,248),fill_value=np.nan) #圈出范围
                ...
                event_all_stats = pd.concat([event_all_stats, pd.Series([i,days[j],lat_1.values,lon_1.values,proj_area,area,amounts, (p_event* p).mean(('lon','lat')).values  ]) ],axis=1)
                
        event_all_stats = event_all_stats.T
        event_all_stats.columns = ['event','day','lat','lon','projection_area','daily_area','daily_amounts','intensity']
        ...
        intensity_new = event_all_stats.iloc[:,:8].groupby('event').mean()['intensity']
        circulation =event_all_stats.groupby('event')['circulation_type'].mean()
        event_basic_stats.index=range(len(event_basic_stats))
        event_basic_stats= pd.concat([event_basic_stats,prec_amount,accu_area,proj_area,intensity,intensity_new,circulation],axis=1)
