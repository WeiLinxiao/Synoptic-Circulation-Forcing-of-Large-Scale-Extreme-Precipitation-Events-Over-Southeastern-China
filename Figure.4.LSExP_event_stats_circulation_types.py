...
for qu in [0.99]:
    for ar in [0.95]:
        ...
        for j in range(6):
            ...
            ##### 面积相关的降水特性
            ax.bar(j-0.08, a.mean()[5],color =cmap1[4*j+2],width=0.16,alpha=0.7,edgecolor= cmap1[4*j+1])
            ax.errorbar(j-0.08, a.mean()[5], yerr=a.std()[5], capsize=3, capthick=0.5,elinewidth=1,color=cmap1[4*j])
            print('area '+str(a.mean()[5]))
            ax.bar(j+0.08, a.mean()[6],color =cmap1[4*j+3],width=0.16,alpha=0.7,edgecolor= cmap1[4*j+2],linestyle='--',label='aa' )
            ax.errorbar(j+0.08, a.mean()[6], yerr=a.std()[6], capsize=3, capthick=0.5,elinewidth=1,color=cmap1[4*j],linestyle='--')
            print('p_area '+str(a.mean()[6]))
            ax2.bar(j+0.32, a.mean()[4],color =cmap1[4*j+1],width=0.16,alpha=0.7,edgecolor= 'black' )
            ax2.errorbar(j+0.32, a.mean()[4], yerr=a.std()[4], capsize=3, capthick=0.5,elinewidth=1,color='black')
            ...
            ax3.bar((uni/360)*2*np.pi,count,alpha=0.9,width=0.51,color=cmap1[4*j+1],edgecolor='black')
        ...
        #
            ...
            for vp in vio1['bodies']:
                vp.set_facecolor(cmap1[4*j+2])
                vp.set_edgecolor(cmap1[4*j+2])
                vp.set_linewidth(1)
                vp.set_alpha(0.5)
            # Make all the violin statistics marks red:
            vp = vio1['cmedians']
            vp.set_edgecolor(cmap1[4*j+1])
            vp.set_facecolor(cmap1[4*j+1])
            vp.set_linewidth(2)
            vp.set_alpha(1)
