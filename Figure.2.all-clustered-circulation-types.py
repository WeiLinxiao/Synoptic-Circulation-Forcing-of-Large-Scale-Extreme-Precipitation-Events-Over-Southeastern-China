...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 28 14:48:45 2023
...
slp=xr.open_dataarray(basic_dir+'research/6.Southeastern_China_Clustering/data/ERA5_variables/ERA5_19790101_20201231_daily_slp.nc').sel(
   longitude=slice(90,140),
   latitude=slice(50,0))/100


z500_anomaly=xr.open_dataarray(basic_dir+'research/6.Southeastern_China_Clustering/data/ERA5_variables/smoothed_anomaly_1979_2019_Z500.nc').sel(
   longitude=slice(90,140),
   latitude=slice(50,0))/9.806
...
slp_anomaly=xr.open_dataarray(basic_dir+'research/6.Southeastern_China_Clustering/data/ERA5_variables/smoothed_anomaly_1979_2019_slp.nc').sel(
   longitude=slice(90,140),
   latitude=slice(50,0))/100
...
fig = plt.figure(figsize = (17.5/2.54, 14/2.54)) # 宽、高

for j in [0,1,2,3,4,5]:

    ax = plt.axes([x_loc[ j  ],y_loc[  j  ],0.34,0.25],projection=ccrs.PlateCarree() )

    time = pattern.iloc[np.where(pattern.values==sequence_f[j])[0]].index  #事件发生的事件
    print(sequence_f[j])
    ...
    cf = ax.contourf(mf_u['longitude'].values, mf_u['latitude'].values, 
                slp_mean.values,cmap=Colors_used_slp,
                transform=ccrs.PlateCarree(),
                levels=Colors_limits_slp)
    
    cont = ax.contour(mf_u['longitude'].values, mf_u['latitude'].values, 
               z500_mean.values,
               color='black',
    ...
    # 7. 一些基本设置
    ax.set_extent((90, 140, 10, 50), ccrs.PlateCarree())
    ax.set_aspect('auto') ##Non-fixed aspect
    ax1.set_title(r"$\bf{("+title_aux[j]+")}$ Cl. "+ str(j+1)+" ("+str(np.round(count_f[sequence_f[j]],1))+"%)",
...
#fig.savefig("/media/dai/DATA1/research/6.Southeastern_China_Clustering/code2/Figures/Figure.2.pdf",dpi=1500, bbox_inches='tight')
fig.savefig("/media/dai/DATA1/research/6.Southeastern_China_Clustering/code2/Figures/Figure.2.png",dpi=1500, bbox_inches='tight')
